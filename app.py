...
def get_funding_wallet_balances():
    try:
        # Attempt to fetch all wallet balances
        ...
        data = response.json()
        logger.info(f"Raw Funding Wallet response: {data}")  # Debug log
        return data
    except Exception as e:
    ...
    try:
        ...
        if funding and isinstance(funding, list):
            for asset in funding:
                funding_free = float(asset.get('fundingFree', 0))
                if funding_free > 0:
                    rewards.append(f"Funding Wallet: {asset['asset']} - {funding_free}")
...
def trading_loop():
    while True:
        try:
            ...
            # Check Funding Wallet for USDT (fallback to manual transfer note)
            funding = get_funding_wallet_balances()
            funding_usdt = next((float(asset.get('fundingFree', 0)) for asset in funding if asset.get('asset') == "USDT"), 0.0)
            logger.info(f"Current Funding USDT balance: {funding_usdt:.6f}")
            if funding_usdt == 0.0:
                logger.warning("Funding Wallet balance not detected—transfer 1+ USDT from Funding to Spot manually when possible.")
            elif funding_usdt >= 0.01:
