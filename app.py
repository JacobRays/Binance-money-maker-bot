...
def get_funding_wallet_balances():
    try:
        # Direct API call to Funding Wallet endpoint
        response = client._request_margin_api('GET', 'asset/wallet/balance', signed=True, data={})
        return response
    except Exception as e:
...
def trading_loop():
    while True:
        try:
            ...
            funding = get_funding_wallet_balances()
            funding_usdt = next((float(asset['free']) for asset in funding if asset['asset'] == "USDT"), 0.0)
